...
class CommandRunner:
    ...
    @staticmethod
    def run(cmd: str, check: bool = True, capture_output: bool = True, 
...
class QualityChecker:
    ...
    def run_code_formatting_check() -> None:
        ...
        if result.returncode != 0:
            Logger.error("Code formatting issues found. Run 'black src/ tests/' to fix.")
            sys.exit(1)
        ...
        for dist in distributions:
            if dist.suffix == '.whl':
                ...
                with zipfile.ZipFile(dist, 'r') as zf:
                    ...
                    if test_files:
                        Logger.warning(f"Found test files in {dist.name}: {test_files[:5]}")
                    else:
                ...
                with tarfile.open(dist, 'r:gz') as tf:
                    files = tf.getnames()
                    test_files = [f for f in files if '/tests/' in f and f.endswith('.py')]
                    if test_files:
                        Logger.warning(f"Found test files in {dist.name}: {test_files[:5]}")
                    else:
                        Logger.success(f"No test files found in {dist.name}")
    ...
    def test_installation() -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if wheel_files:
                ...
                # Test imports
                CommandRunner.run(f"{python_path} -c 'import src; print(f\"Version: {{src.__version__}}\")'")
                
                # Test console scripts
                result = CommandRunner.run(f"{python_path} -c 'import pkg_resources; print([ep.name for ep in pkg_resources.iter_entry_points(\"console_scripts\")])'", check=False)
...
class PyPIUploader:
    ...
    def upload_to_pypi(test_pypi: bool = False) -> None:
        ...
        if not config_file.exists():
            Logger.warning("No .pypirc found. You may need to configure authentication.")
        
        # Upload
        if test_pypi:
            CommandRunner.run("twine upload --repository testpypi dist/*")
            Logger.success("Uploaded to Test PyPI")
            Logger.info("Install with: pip3` install --index-url https://test.pypi.org/simple/ pcileech-fw-generator")
        else:
...
class PackageGenerator:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            Logger.success(f"Package generation completed successfully in {elapsed_time:.1f}s")
