...
class ConfigSpaceManager:
    ...
    def generate_synthetic_config_space(self) -> bytes:
        ...
        # Reserved
        config_space[52:60] = (0x0000000000000000).to_bytes(8, "little")
        ...
        # Cache Line Size, Latency Timer, Header Type, BIST - use device config if available
        if self.device_config:
            config_space[12] = self.device_config.registers.cache_line_size
            config_space[13] = self.device_config.registers.latency_timer
            config_space[14] = self.device_config.registers.header_type
            config_space[15] = self.device_config.registers.bist
        else:
