...
# ───── Ioctl numbers – now computed, never hard-coded ──────────────────────
VFIO_GET_API_VERSION = _IO(VFIO_TYPE, VFIO_BASE + 0)
VFIO_CHECK_EXTENSION = _IO(VFIO_TYPE, VFIO_BASE + 1)
VFIO_SET_IOMMU = _IO(VFIO_TYPE, VFIO_BASE + 2)
VFIO_GROUP_GET_STATUS = _IOR(VFIO_TYPE, VFIO_BASE + 3, vfio_group_status)
VFIO_GROUP_SET_CONTAINER = _IO(VFIO_TYPE, VFIO_BASE + 4)
VFIO_GROUP_UNSET_CONTAINER = _IO(VFIO_TYPE, VFIO_BASE + 5)
# VFIO_GROUP_GET_DEVICE_FD takes a device name string as argument
# In the kernel, this is defined as taking a char name[40] parameter
VFIO_GROUP_GET_DEVICE_FD = _IOW(VFIO_TYPE, VFIO_BASE + 6, ctypes.c_char * 40)
VFIO_DEVICE_GET_INFO = _IO(VFIO_TYPE, VFIO_BASE + 7)
VFIO_DEVICE_GET_REGION_INFO = _IOWR(VFIO_TYPE, VFIO_BASE + 8, vfio_region_info)
