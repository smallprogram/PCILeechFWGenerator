...
def choose_device(devices: List[Dict[str, str]]) -> Dict[str, str]:
    ...
    from .device_prioritizer import CLIDevicePrioritizer, format_device_with_priority

    prioritizer = CLIDevicePrioritizer()

    # Sort devices to put priority ones first
    sorted_devices = prioritizer.sort_devices_by_priority(devices)

    print("\nSelect donor PCIe device:")
    print("\033[92m✓ = Recommended (Network/Storage/Media devices)\033[0m")
    print()

    for i, device in enumerate(sorted_devices):
        formatted_device = format_device_with_priority(device, prioritizer)
        print(f" [{i}] {formatted_device}")
...
def choose_board() -> str:
    ...
    # Import board config to get supported boards with display info
    try:
        from src.device_clone.board_config import (
            get_board_display_info,
            list_boards_with_recommendations,
        )

        boards_with_info = list_boards_with_recommendations()
        supported_boards = [board for board, _ in boards_with_info]
    except ImportError:
        # Fallback to hardcoded list if import fails
        logger.warning("Could not import board configuration, using fallback list")
        supported_boards = [
            "pcileech_75t484_x1",
            "35t",
            "75t",
        ...
        boards_with_info = None

    print("\nSelect target FPGA board:")
    for i, board in enumerate(supported_boards):
        description = board

        # Use display info if available
        if boards_with_info:
            try:
                display_info = get_board_display_info(board)
                description = display_info.get("display_name", board)
                board_desc = display_info.get("description", "")
                if board_desc:
                    description += f" {board_desc}"
            except Exception as e:
                logger.debug(f"Could not get display info for {board}: {e}")
        else:
            # Fallback for hardcoded recommendation
    ...
    default_board = supported_boards[0] if supported_boards else "pcileech_75t484_x1"
    if boards_with_info:
        try:
            default_display_info = get_board_display_info(default_board)
            default_desc = default_display_info.get("display_name", default_board)
        except:
            default_desc = default_board
    else:
        default_desc = (
            "CaptainDMA 75T" if default_board == "pcileech_75t484_x1" else default_board
        )

    print(f"\nDefault: [0] {default_desc} ({default_board})")
    while True:
        try:
...
def create_build_config_from_args(args: argparse.Namespace) -> BuildConfig:
    ...
    return BuildConfig(
        ...
        strict_vfio=getattr(args, "strict_vfio", True),
...
def setup_build_parser(subparsers) -> argparse.ArgumentParser:
    ...
    # Board selection - get dynamic list of supported boards
    try:
    ...
    build_parser.add_argument(
        "--strict-vfio",
        action="store_true",
        help="Require VFIO to work - fail hard if VFIO is not available (ensures unique firmware)",
    )
