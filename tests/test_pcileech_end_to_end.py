...
try:
    ...
    from src.templating.systemverilog_generator import SystemVerilogGenerator, PCILeechOutput
    ...
    class TCLBuilder:
        ...
        def build_pcileech_project_script(self, context): return "# Mock project script"
        def build_pcileech_build_script(self, context): return "# Mock build script"
    ...
    class SystemVerilogGenerator:
        def __init__(self, output_dir): pass
        def discover_and_copy_all_files(self, device_info, pcileech_output): return []
    ...
    class PCILeechOutput:
        def __init__(self, **kwargs):
            ...
            self.generate_explicit_file_lists = kwargs.get("generate_explicit_file_lists", True)
...
CONFIG.PCIe_Compatible {{true}}"""
    ...
    def test_pcileech_2_script_approach_validation(self, temp_dir, pcileech_workflow_config):
...
module {advanced_file.replace('.sv', '')} (
    ...
    // Advanced feature implementation compatible with PCILeech
            ...
            written_file = file_manager.write_to_src_directory(advanced_file, test_content)
            assert written_file.exists()
